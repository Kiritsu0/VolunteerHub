    ...
    <div>
      <header className="flex justify-between items-center w-screen min-h-14">
        ...
        {pathname === "/home/volunteer" ? 
          <Link href="" className="relative cursor-pointer group overflow-hidden">
            <h2 className="font-medium mb-2 sm:text-1xl md:text-2xl lg:text-2xl">Joined Events</h2>
          ...
          <div className="flex gap-8">
            <Link href="" className="relative cursor-pointer group overflow-hidden">
              <h2 className="font-medium mb-2 sm:text-1xl md:text-2xl lg:text-2xl">Created Events</h2>
              <div className="absolute bottom-0 left-0 w-full h-1 bg-green-600 transition-transform transform translate-x-60 group-hover:translate-x-0"></div>
            </Link>
            <Link href="" className="relative cursor-pointer group overflow-hidden">
              <h2 className="font-medium mb-2 sm:text-1xl md:text-2xl lg:text-2xl">Create Event</h2>
              <div className="absolute bottom-0 left-0 w-full h-1 bg-green-600 transition-transform transform translate-x-60 group-hover:translate-x-0"></div>
            </Link>
          </div>
        }
          ...
          <div onClick={dropdown} className="group relative flex items-center gap-3 border-slate-400 border-2 shadow-md shadow-slate-500 cursor-pointer py-1 px-2 rounded-3xl hover:shadow-xl">
            ...
            { showDropdown && (
              <div className="absolute top-12 left-1/2 transform -translate-x-1/2 bg-white text-sm h-48 w-40 rounded-md">
                ...
                <hr />
                <Link href="" className="flex gap-2 items-center p-2 hover:bg-slate-200">
